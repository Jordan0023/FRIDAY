...
from netgear_firmware_audit.glinet import download_glinet_router_firmware


def main() -> int:
    parser = argparse.ArgumentParser(description="Download GL.iNet router firmware into known_firmware.")
    ...
    parser.add_argument("--resume", action="store_true", help="Skip GL.iNet products already recorded in the manifest.")
    ...
    if args.include_beta:
        stages.update({"TESTING", "RC"})
    if args.include_snapshot:
        stages.add("SNAPSHOT")

    products, firmware, downloaded = download_glinet_router_firmware(
        root=args.root,
    ...
    print(f"GL.iNet products checked: {products}")
    print(f"GL.iNet firmware links found: {firmware}")
